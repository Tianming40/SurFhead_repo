...
def world_to_clip(vtx, RT, K, image_size, mvp=None):
...
def projection_from_intrinsics( K: torch.Tensor, image_size: Tuple[int], near: float=0.1, far:float=10):
    """
...
def compute_face_normals(verts, faces):
    ...
    return face_normals.squeeze(), face_centroids.squeeze()
...
def to_cam_open3d(viewpoint_stack):
    ...
    for i, viewpoint_cam in enumerate(viewpoint_stack):
        ...
        intrins =  (viewpoint_cam.projection_matrix.cuda() @ ndc2pix)[:3,:3].T
...
class GaussianExtractor(object):
    ...
    def reconstruction(self, viewpoint_stack):
        ...
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
            ...
            # si(rgb, f"analysis_results/rgb_{i}.png")
    ...
    def extract_mesh_bounded(self, voxel_size=0.004, sdf_trunc=0.02, depth_trunc=3, mask_backgrond=True):
        ...
        for i, cam_o3d in tqdm(enumerate(to_cam_open3d(self.viewpoint_stack)), desc="TSDF integration progress"):
            ...
            if mask_backgrond and (self.viewpoint_stack[i].original_mask is not None):
                depth[(self.viewpoint_stack[i].original_mask < 0.5)] = 0
