...
def get_minimum_axis(scales, rotations):
    #! input: s and R(norm == 1, unit quaternion)
    # breakpoint()
    # sorted_idx = torch.argsort(scales, descending=False, dim=-1)
    # R = build_rotation(rotations)#! rotation denotes the direction 3orthonormal direction!
    ...
    x_axis = rotations_mat[indices, :, min_scales]
    # breakpoint()
    return x_axis
...
def flip_align_view(normal, viewdir):
    ...
    normal_flipped = normal*torch.where(non_flip, 1, -1) # (N, 3) #! flipping the axis with the dotprod<0(opposite direction of viewdir)
