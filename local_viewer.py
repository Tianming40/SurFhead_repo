        ...
        with dpg.window(label="Render", tag="_render_window", autosize=True):
            ...
            with dpg.group(horizontal=True):
                ...
                if self.gaussians.binding is not None:
                    ...
                    def callback_show_depth(sender, app_data):
                        self.need_update = True
                    dpg.add_checkbox(label="show depth", default_value=False, callback=callback_show_depth, tag="_checkbox_show_depth")

                    def callback_show_index_enable(sender, app_data):
                        self.need_update = True
                    dpg.add_checkbox(label="show face_index", default_value=False, callback=callback_show_index_enable, tag="_checkbox_show_face_index")
            ...
            with dpg.group(horizontal=True):
                def callback_show_normal_short(sender, app_data):
                    self.need_update = True
                dpg.add_checkbox(label="show normal_short", default_value=False, callback=callback_show_normal_short, tag="_checkbox_show_normal_short")
                
                def callback_show_normal_neigh(sender, app_data):
                    self.need_update = True
                dpg.add_checkbox(label="show normal_neigh", default_value=False, callback=callback_show_normal_neigh, tag="_checkbox_show_normal_neigh")
                
                def callback_show_opacity(sender, app_data):
                    self.need_update = True
                dpg.add_checkbox(label="show opacity_1", default_value=False, callback=callback_show_opacity, tag="_checkbox_show_opacity=1")
            ...
            with dpg.group(horizontal=True):   
                def callback_set_face_index(sender, app_data):
                    ...
                    # # self.gaussians.select_mesh_by_timestep(self.timestep)
                    self.need_update = True
                dpg.add_slider_int(label="face_index", tag='_slider_face_index', width=153, min_value=0, max_value=10142, format="%d", default_value=0, callback=callback_set_face_index)
            
            with dpg.group(horizontal=True):
                def callback_show_bf_culling(sender, app_data):
                    self.need_update = True
                dpg.add_checkbox(label="show backface_culling", default_value=False, callback=callback_show_bf_culling, tag="_checkbox_show_bf_culling")
        ...
        while dpg.is_dearpygui_running():
            ...
            if self.need_update or self.playing:
                ...
                if dpg.get_value("_checkbox_show_splatting") and dpg.get_value("_checkbox_show_mesh"):
                    ...
                    rgb = rgb_mesh * alpha_mesh * mesh_opacity  + rgb_depth * (alpha_mesh * (1 - mesh_opacity) + (1 - alpha_mesh))
                elif dpg.get_value("_checkbox_show_normal_short") and dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_mesh * alpha_mesh * mesh_opacity  + rgb_normal * (alpha_mesh * (1 - mesh_opacity) + (1 - alpha_mesh))
                elif dpg.get_value("_checkbox_show_normal_neigh") and dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_mesh * alpha_mesh * mesh_opacity  + rgb_neigh_normal * (alpha_mesh * (1 - mesh_opacity) + (1 - alpha_mesh))
                elif dpg.get_value("_checkbox_show_opacity=1") and dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_mesh * alpha_mesh * mesh_opacity  + rgb_opacity * (alpha_mesh * (1 - mesh_opacity) + (1 - alpha_mesh))
                elif dpg.get_value("_checkbox_show_bf_culling") and dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_mesh * alpha_mesh * mesh_opacity  + rgb_bf_culling * (alpha_mesh * (1 - mesh_opacity) + (1 - alpha_mesh))
                    ...
                    mesh_opacity = self.mesh_color[3:].cuda()
                    mesh_opacity /= 2
                    rgb = rgb_mesh * alpha_mesh * mesh_opacity  + rgb_face_index * (alpha_mesh * (1 - mesh_opacity) + (1 - alpha_mesh))
