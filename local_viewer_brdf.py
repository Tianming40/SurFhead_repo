...
class LocalViewer(Mini3DViewer):
    ...
    def init_gaussians(self):
        ...
        if motion_path.exists():
            self.gaussians = FlameGaussianModel(sh_degree=-1, sg_degree=24, brdf_dim=3,
                                                brdf_mode="envmap",
                                                brdf_envmap_res=64)
        else:
        ...
        with dpg.window(label="Render", tag="_render_window", autosize=True):
            ...
            with dpg.group(horizontal=True):
                ...
                if self.gaussians.binding is not None:
                    ...
                    def callback_show_depth(sender, app_data):
                        self.need_update = True

                    dpg.add_checkbox(label="show depth", default_value=False, callback=callback_show_depth,
                                     tag="_checkbox_show_depth")

                    def callback_show_index_enable(sender, app_data):
                        self.need_update = True

                    dpg.add_checkbox(label="show face_index", default_value=False, callback=callback_show_index_enable,
                                     tag="_checkbox_show_face_index")
            ...
            with dpg.group(horizontal=True):
                def callback_show_normal_short(sender, app_data):
                    self.need_update = True

                dpg.add_checkbox(label="show normal_short", default_value=False, callback=callback_show_normal_short,
                                 tag="_checkbox_show_normal_short")

                def callback_show_normal_neigh(sender, app_data):
                    self.need_update = True

                dpg.add_checkbox(label="show normal_neigh", default_value=False, callback=callback_show_normal_neigh,
                                 tag="_checkbox_show_normal_neigh")

                def callback_show_opacity(sender, app_data):
                    self.need_update = True

                dpg.add_checkbox(label="show opacity_1", default_value=False, callback=callback_show_opacity,
                                 tag="_checkbox_show_opacity=1")

            with dpg.group(horizontal=True):
                def callback_set_face_index(sender, app_data):
                    # if sender == "_slider_timestep":
                    #     self.face_index = app_data
                    # elif sender in ["_button_timestep_plus", "_mvKey_Right"]:
                    #     self.face_index = min(self.face_index + 1, self.num_face_index - 1)
                    # elif sender in ["_button_timestep_minus", "_mvKey_Left"]:
                    #     self.face_indexp = max(self.face_index - 1, 0)
                    # elif sender == "_mvKey_Home":
                    #     self.face_index = 0
                    # elif sender == "_mvKey_End":
                    #     self.face_index = self.num_face_index - 1

                    # dpg.set_value("_slider_face_index", self.face_index)
                    # # self.gaussians.select_mesh_by_timestep(self.timestep)
                    self.need_update = True

                dpg.add_slider_int(label="face_index", tag='_slider_face_index', width=153, min_value=0,
                                   max_value=10142, format="%d", default_value=0, callback=callback_set_face_index)

            with dpg.group(horizontal=True):
                def callback_show_bf_culling(sender, app_data):
                    self.need_update = True

                dpg.add_checkbox(label="show backface_culling", default_value=False, callback=callback_show_bf_culling,
                                 tag="_checkbox_show_bf_culling")
        ...
        with dpg.handler_registry():
            ...
            def callbackmouse_wheel_slider(sender, app_data):
                ...
                if dpg.is_item_hovered("_slider_timestep"):
                    ...
                    self.gaussians.select_mesh_by_timestep(self.timestep)
                    self.need_update = True
                if dpg.is_item_hovered("_slider_face_index"):
                    self.timestep = min(max(self.timestep - delta, 0), self.num_timesteps - 1)
                    dpg.set_value("_slider_timestep", self.timestep)
                    self.gaussians.select_mesh_by_timestep(self.timestep)
                    self.need_update = True
    ...
    def prepare_nn_camera(self):
        @dataclass
        class Cam:
            cam: any  # 이 부분은 적절한 타입으로 변경 필요
            FoVx: float = field(init=False)
            FoVy: float = field(init=False)
            ...
            def __post_init__(self):
                self.FoVx = float(np.radians(self.cam.fovx))
                self.FoVy = float(np.radians(self.cam.fovy))
                self.image_height = self.cam.image_height
                self.image_width = self.cam.image_width
                self.world_view_transform = torch.tensor(self.cam.world_view_transform).float().cuda().T
                self.full_proj_transform = torch.tensor(self.cam.full_proj_transform).float().cuda().T
                self.camera_center = torch.tensor(self.cam.pose[:3, 3]).cuda()

            def get_calib_matrix_nerf(self):
                focal = fov2focal(self.FoVx, self.image_width)
                intrinsic_matrix = torch.tensor(
                    [[focal, 0, self.image_width / 2], [0, focal, self.image_height / 2], [0, 0, 1]]).float()
        ...
        while dpg.is_dearpygui_running():
            ...
            if self.need_update or self.playing:
                ...
                if dpg.get_value("_checkbox_show_splatting"):
                    ...
                    rgb_splatting = rgb_splatting.permute(1, 2, 0).contiguous()
                    # opacity
                ...
                if dpg.get_value("_checkbox_show_opacity=1"):
                    # rgb
                ...
                if dpg.get_value("_checkbox_show_depth"):
                    ...
                    # breakpoint()
                    rgb_depth = apply_depth_colormap(rgb_depth[0][..., None]).permute(2, 0, 1).permute(1, 2,
                                                                                                       0).contiguous()

                if dpg.get_value("_checkbox_show_normal_short"):
                    # normal_short
                ...
                if dpg.get_value("_checkbox_show_splatting") and dpg.get_value("_checkbox_show_mesh"):
                    ...
                    rgb = rgb_splatting
                elif dpg.get_value("_checkbox_show_depth") and dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_mesh * alpha_mesh * mesh_opacity + rgb_depth * (
                                alpha_mesh * (1 - mesh_opacity) + (1 - alpha_mesh))
                elif dpg.get_value("_checkbox_show_normal_short") and dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_mesh * alpha_mesh * mesh_opacity + rgb_normal * (
                                alpha_mesh * (1 - mesh_opacity) + (1 - alpha_mesh))
                elif dpg.get_value("_checkbox_show_normal_neigh") and dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_mesh * alpha_mesh * mesh_opacity + rgb_neigh_normal * (
                                alpha_mesh * (1 - mesh_opacity) + (1 - alpha_mesh))
                elif dpg.get_value("_checkbox_show_opacity=1") and dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_mesh * alpha_mesh * mesh_opacity + rgb_opacity * (
                                alpha_mesh * (1 - mesh_opacity) + (1 - alpha_mesh))
                elif dpg.get_value("_checkbox_show_bf_culling") and dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_mesh * alpha_mesh * mesh_opacity + rgb_bf_culling * (
                                alpha_mesh * (1 - mesh_opacity) + (1 - alpha_mesh))
                elif not dpg.get_value("_checkbox_show_splatting") and dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_mesh
                elif dpg.get_value("_checkbox_show_bf_culling") and not dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_bf_culling
                elif dpg.get_value("_checkbox_show_depth") and not dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_depth
                elif dpg.get_value("_checkbox_show_normal_short") and not dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_normal
                elif dpg.get_value("_checkbox_show_normal_neigh") and not dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_neigh_normal
                elif dpg.get_value("_checkbox_show_opacity=1") and not dpg.get_value("_checkbox_show_mesh"):
                    rgb = rgb_opacity
                elif dpg.get_value("_checkbox_show_face_index") and not dpg.get_value("_checkbox_show_mesh"):
                    out_dict = self.mesh_renderer.render_from_camera(self.gaussians.verts, self.gaussians.faces, cam,
                                                                     face_colors=self.face_colors)
                    rgba_mesh = out_dict['rgba'].squeeze(0)  # (H, W, C)
                    rgb_mesh = rgba_mesh[:, :, :3]
                    ...
                    mesh_opacity = self.mesh_color[3:].cuda()
                    mesh_opacity /= 2
                    rgb = rgb_mesh * alpha_mesh * mesh_opacity + rgb_face_index * (
                                alpha_mesh * (1 - mesh_opacity) + (1 - alpha_mesh))
                elif dpg.get_value("_checkbox_show_face_index"):
                    if dpg.get_value("_checkbox_show_mesh"):
                        out_dict = self.mesh_renderer.render_from_camera(self.gaussians.verts, self.gaussians.faces,
                                                                         cam, face_colors=self.face_colors)
                        rgba_mesh = out_dict['rgba'].squeeze(0)  # (H, W, C)
                        rgb_mesh = rgba_mesh[:, :, :3]
                        alpha_mesh = rgba_mesh[:, :, 3:]
                        mesh_opacity = self.mesh_color[3:].cuda()
