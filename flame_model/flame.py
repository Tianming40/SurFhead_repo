...
#! added 0511
FLAME_MESH_PATH_2020 = "flame_model/assets/flame2020/head_template.obj"
FLAME_LMK_PATH_2020 = "flame_model/assets/flame2020/landmark_embedding_with_eyes.npy"
FLAME_MODEL_PATH_2020 = "flame_model/assets/flame2020/generic_model.pkl"
...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        if self.n_shape_params == 100 and self.n_expr_params == 50:
            flame_model_path = FLAME_MODEL_PATH_2020
            flame_lmk_embedding_path = FLAME_LMK_PATH_2020
            flame_template_mesh_path = FLAME_MESH_PATH_2020
        else:
            #! assert with message
            assert self.n_shape_params == 300 and self.n_expr_params == 100, \
                    f"Which version of FLAME you use?? Check the shape_params: {self.n_shape_params}, expr_params: {self.n_expr_params}"
    ...
    def compute_face_laplacian(self):
        # verts: N x 3 tensor of vertex positions
        # faces: M x 3 tensor of indices into verts, defining the triangular faces
        ...
        num_faces = self.faces.shape[0]
        num_vertices = self.faces.max().item() + 1

        # 각 정점에 연결된 면의 인덱스를 저장하는 리스트
        vertex_to_faces = [[] for _ in range(num_vertices)]
        for face_idx, face in enumerate(self.faces):
            for vertex in face:
                vertex_to_faces[vertex].append(face_idx)

        # 인접 행렬 초기화 (num_faces x num_faces)
        adjacency_matrix = torch.zeros((num_faces, num_faces), dtype=torch.int32)
        ...
        for face_idx, face in enumerate(self.faces):
            ...
            adjacent_faces.remove(face_idx)  # 자기 자신은 제외
            for adj_face_idx in adjacent_faces:
                adjacency_matrix[face_idx, adj_face_idx] = 1
        # breakpoint()
        self.face_sharing = adjacency_matrix
        ...
        # breakpoint()
        # I = torch.hstack([self.faces[:, 0], self.faces[:, 1], self.faces[:, 2]])
        # J = torch.hstack([self.faces[:, 1], self.faces[:, 2], self.faces[:, 0]])
        # V = torch.ones(I.shape[0], device=verts.device)
        
        # adjacency_matrix = torch.sparse_coo_tensor(indices=torch.vstack([I, J]), values=V, size=(num_verts, num_verts))
        ...
        # # Laplacian matrix
        # laplacian_matrix = degree_matrix - adjacency_matrix.to_dense()
        # self.L_mat = laplacian_matrix
        # self.D_mat = degree_matrix
        # self.A_mat = adjacency_matrix.to_dense()
    ...
    def forward(
        ...
        betas_cano = torch.cat([shape, torch.zeros_like(expr).to(expr)], dim=1)
        
        full_pose_cano = torch.zeros_like(full_pose)
        # full_pose_cano[:, 6] = 1 #! to open mouth
        
        v_shaped_canonical = template_vertices + blend_shapes(betas_cano, self.shapedirs)
        v_shaped_canonical ,_,_ = lbs(
            full_pose_cano,
            v_shaped_canonical,
            self.posedirs,
            self.J_regressor,
        ...
        v_shaped_canonical = v_shaped_canonical + translation[:, None, :]
        
        # v_shaped_canonical = v_shaped
        # Add personal offsets
        if static_offset is not None:
            v_shaped += static_offset
            v_shaped_canonical += static_offset
        ...
        if return_verts_cano:
            ...
            ret_vals.append(v_shaped_canonical)
...
class FlameMask(nn.Module):
    ...
    def create_custom_mask(self):
        ...
        self.v.register_buffer("sclerae", sclerae) #! 흰자
