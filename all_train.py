    ...
    if dataset.bind_to_mesh:
        ...
        gaussians = FlameGaussianModel(dataset.sh_degree, dataset.sg_degree,brdf_dim=dataset.brdf_dim,
            ...
            brdf_envmap_res=dataset.brdf_envmap_res, disable_flame_static_offset= dataset.disable_flame_static_offset,
                                       not_finetune_flame_params= dataset.not_finetune_flame_params, n_shape=n_shape, n_expr=n_expr,
...
def training_report(tb_writer, iteration, losses, elapsed, testing_iterations, scene: Scene, renderFunc, renderArgs,
    ...
    if iteration in testing_iterations:  # or iteration in [5000, 10000]:
                ...
                for idx, viewpoint in tqdm(
                    ...
                    if tb_writer and (idx % (len(config['cameras']) // num_vis_img) == 0):
                        ...
                        try:
                            ...
                            surf_normal = (render_pkg["surfel_surf_normal"] * 0.5 + 0.5) * gt_mask.repeat(3, 1, 1).cuda + (
        ...
        if tb_writer:
            ...
            try:
                tb_writer.add_histogram('scene/blend_weight_primary', F.normalize(scene.gaussians.get_blend_weight, dim=-1)[..., :1], iteration)
            except:
                pass
        torch.cuda.empty_cache()
