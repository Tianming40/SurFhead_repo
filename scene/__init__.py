...
class CameraDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ...
            # breakpoint()
            if camera.mask_face is None:
                mask_face = Image.open(camera.mask_face_path)
            else:
                mask_face = camera.mask_face
            ...
            resized_mask_face_gray = PILtoTorch(mask_face, (camera.image_width, camera.image_height))[0:1, ...]
            
            image = resized_image_rgb[:3, ...]
            mask = resized_mask_gray
            mask_face = resized_mask_face_gray
            # breakpoint()
            ...
            camera.original_mask_face = mask_face.clamp(0.0, 1.0)
            return camera
...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : Union[GaussianModel, FlameGaussianModel], load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        # process meshes
        # breakpoint()
        if gaussians.binding != None: #! here!!!!
...
class RelightingScene:
    gaussians: GaussianModel

    def __init__(self, args: ModelParams, gaussians: Union[GaussianModel, FlameGaussianModel], load_iteration=None,
